...
borrow_association_table = db.Table('run_to_borrow_characters', db.metadata,
    db.Column('borrow_run_id', db.Integer, ForeignKey('run.id')),
    db.Column('borrow_guild_characters_id', db.Integer, ForeignKey('guild_characters.id'))
)
...
def treasury():
  ...
  treasures_transactions = db.session.query(MappedGuildTreasure, MappedGuildTransaction, MappedPlayer).outerjoin(MappedGuildTransaction).outerjoin(MappedPlayer).order_by(MappedGuildTreasure.itemid).all()
  ...
  try:
    ...
    for c in chars:
      ...
      if found_chars:
        ...
        mc.Class = c.Class
        mc.Name = c.Name
        mc.LastRun = c.LastRun
        mc.PlayerName = c.PlayerName
...
def CalculatePoints(run = None, mobs_killed = [], players = [], borrow_players = [], market_results = {}, d = datetime.now()): 
  ...
  if len(run_credits) > 0 and len(mapped_guild_points) > 0:
    ...
    for player_id in player_ids:
      ...
      print 'RC player ID: {0} current player ID: {1} RC run ID: {2} current run ID: {3}'.format(rcs.player_id, player_id, rcs.run_id, run_id)
      ...
      print 'MGP player ID: {0} current player ID: {1} MGP run ID: {2} current run ID: {3}'.format(mgp.player_id, player_id, mgp.run_id, run_id)
