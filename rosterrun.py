...
def item_history():
  ...
  projected_results = [(convert_to_key(m.itemid, m.name, m.cards, m.date.strftime('%w, %b %Y')), {'value': int(m.price), 'label':convert_to_key(m.itemid, m.name, m.cards, m.date.strftime('%d, %b %Y'))}) for m in mrs]
  print projected_results
  res_dict = {}
  for key, group in groupby(projected_results, lambda x: x[0]):
    for pr in group:
  ...
  [bar_chart.add('MIN ' + k, {'label' : min(res_dict[k]['label']), 'value': res_dict[k]['label']}) for k in res_dict.keys()]
  [bar_chart.add('MAX ' + k, {'label' : max(res_dict[k]['label']), 'value': res_dict[k]['label']}) for k in res_dict.keys()]
  [bar_chart.add('MEDIAN ' + k, {'label' : median(res_dict[k]['label']), 'value': res_dict[k]['label']}) for k in res_dict.keys()]
