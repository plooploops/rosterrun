...
def item_history():
  ...
  projected_results = [(convert_to_key(m.name), {'value':(m.date, int(m.price)), 'label':convert_to_key(m.itemid, m.name, m.cards)}) for m in mrs]  
  ...
  projected_results = [(convert_to_key(m.name, m.cards), {'value': int(m.price), 'label':convert_to_key(m.itemid, m.name, m.cards, m.date.strftime('%d, %b %Y'))}) for m in mrs]
  print projected_results
  res_dict = {}
  for key, group in groupby(projected_results, lambda x: x[0]):
    for pr in group:
