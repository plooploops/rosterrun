...
def show_entries():
    ...
    if(session['doc'] is not None and len(session['doc']) > 0):
      ...
      cur = g.db.execute('select partyIndex, instanceName, playername, name, class, rolename from combinations where g_spreadsheet_id = ? and g_worksheet_id = ? order by partyIndex, instanceName desc', \
